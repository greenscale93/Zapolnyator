...
конфликту и падению Actions (race condition).

**Если Actions упал:**
1. **Не лезть на VPS вручную** — не чинить контейнеры руками
2. Посмотреть логи упавшего Actions через GitHub (что пошло не так)
3. Исправить ошибку в коде
4. Сделать `commit --amend` или новый `commit` → `push`
5. Actions перезапустится автоматически с исправлением
